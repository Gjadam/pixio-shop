...
export default function Shop() {
    ...
    const [products, setProducts] = useState(allProducts.allProductsData)
    useEffect(() => {
    ...
    const nextProductsPageHandler = () => {
        if (allProducts.next) {
            fetch(allProducts.next)
                .then(res => res.json())
                .then(data => {
    ...
    const previousProductsPageHandler = () => {
        if (allProducts.previous) {
            fetch(allProducts.previous)
                .then(res => res.json())
                .then(data => {
                    setProducts(data.results)
                })
            ...
            <div className=" relative flex justify-between items-start px-28 py-10">
                ...
                <div className=" w-full lg:mx-24">
                    ...
                    <div className=" flex justify-center lg:justify-between  items-center flex-wrap gap-10 w-full mt-10">
                        ...
                        <div className="">
                            <span className=' mx-1 border-1 border-black rounded-full px-3 py-1 lg:px-6 lg:py-2 hover:bg-black hover:text-white cursor-pointer transition-colors duration-300' onClick={previousProductsPageHandler}>PREV</span>
                            <span className=' mx-1 border-1 border-black rounded-full px-3 py-1 lg:px-6 lg:py-2 hover:bg-black hover:text-white cursor-pointer transition-colors duration-300' onClick={nextProductsPageHandler}>NEXT</span>
                        </div>
                    </div>
                </div>
            </div>
